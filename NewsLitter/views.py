...
def newsletter_singup(request):
    ...
    if form.is_valid():
        ...
        if NewsLetterUser.objects.filter(email=instance.email).exists():
            ...
            messages.warning(request,f'you already subscribe')
        else:
            # if email not register save this email and show messages success
            instance.save()
            messages.success(request,f'you subscribe is done')
            #l am use function send() to send email
            subject = "Miling list for simolyfi blog"
...
def newsletter_unsubscribe(request):
    ...
    if form.is_valid():
        ...
        if NewsLetterUser.objects.filter(email=instance.email).exists():
            ...
            messages.success(request,f'your subscription is cansled')
           ...
           # already cleared from the table and then I send a farewell message"""
            ...
            messages.warning(request,f'this email is not exists')
    context ={
        'newslatterform': form
    }   
...
def control_newsletter(request):
    ...
    return render(request, 'control_newsletter.html', {'newscreateform': form})